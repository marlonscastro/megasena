...
int main(int argv, char *argc[])
{
    ...
    int i, j, k;
    int *numeros;
    int num[6] = {0,0,0,0,0,0}, ocorr[6] = {0,0,0,0,0,0};
    ...
    strcat(fileName, "DezenasSeguidasAnalise.txt");
    f = fopen(fileName, "w");
    if(!f){
    ...
    for(i = 1; i <= 1889; i++){
        ...
        printf("analisando jogo %04d | %02d - %02d - %02d - %02d - %02d - %02d", i, num[0], num[1], num[2], num[3], num[4], num[5]);
        fprintf(f, "analisando jogo %04d | %02d - %02d - %02d - %02d - %02d - %02d", i, num[0], num[1], num[2], num[3], num[4], num[5]);
        contNum = 0;
        total = 0;
        for(j = 0; j < 6; j++){
            if((num[j]>=21) && (num[j]<31)){
                ocorr[contNum++] = num[j];
            }
        }

        for(j = 0;j < contNum; j++){
            if(ocorr[j] == (ocorr[j+1]-1))
                total++;
        }
