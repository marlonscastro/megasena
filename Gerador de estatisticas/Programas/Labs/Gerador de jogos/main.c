...
void ordenaJogo(){
    int i, j, aux;
    for( i=0; i<numDezenas-1; i++ ){
        for( j=i+1; j<numDezenas; j++ ){
...
int main()
{
    ...
    int distribuicao[60] = {10,5,53,23,4,33,54,24,51,42,17,37,27,52,28,
                            ...
                            39,7,14,03,19,60,25,48,57,15,9,22,55,21,26}; // utilizado para verificar duplicidade em grupos 5 MCM
    contGeral=0;
    memset(dezenasProibidas, 0, 6);
    contDezenasProibidas=0;
    _maisCont = _comunsCont = _menosCont = 0;

        // ---------- Parametros para 7 dezenas
        //               +    .    -   Par  <31  proporcao
    int param[12][6] = {{3   ,2   ,2   ,4   ,4   ,9},  // ex: 9%
                        {3   ,2   ,2   ,4   ,3   ,11}, // ex: 11%
                        {3   ,2   ,2   ,3   ,5   ,9},
                        {3   ,2   ,2   ,3   ,3   ,11},
                        {2   ,3   ,2   ,3   ,3   ,7},
    ...
    /*    // ---------- Parametros para 8 dezenas
        //               +    .    -   Par  <31  proporcao
    int param[12][6] = {{3   ,3   ,2   ,4   ,4   ,9},  // ex: 9%
                        {3   ,3   ,2   ,4   ,3   ,11}, // ex: 11%
                        {3   ,3   ,2   ,3   ,5   ,9},
                        {3   ,3   ,2   ,3   ,3   ,11},
                        {2   ,4   ,2   ,4   ,3   ,7},
    ...
    /* // ---------- Parametros para 9 dezenas
    int param[12][6] = {{3   ,3   ,3   ,5   ,5   ,9},  // ex: 9%
                        {3   ,3   ,3   ,5   ,3   ,11}, // ex: 11%
                        {3   ,3   ,3   ,4   ,6   ,9},
                        {3   ,3   ,3   ,4   ,3   ,11},
                        {3   ,4   ,2   ,5   ,4   ,7},
    ...
    while(contSeq<12){
        ...
        while(quant < (int)(((float)param[contSeq][5])/100*quantidadeDezenas)){
            ...
            for(j = 0; j < 60; j+=5){   // TESTADO OK: Mas em alguns casos acontece dupla duplicidade.. Verificar Estatisticas!
                existeDupla = 0;
                for(k = 0; k < numDezenas; k++){
                    for(x = 0; x < numDezenas-2; x++){
            ...
            for(i = 0; i < (sizeof(jogosMega)/24); i++){
                for(j=0;j<numDezenas-1;j++) sorteioTemp[j] = jogosMega[i][j];
                contDezenas = 0;
                for(j = 0; j < numDezenas-1; j++){
            ...
            int duques = 0;
            int ternos = 0;
            int tempJogo[numDezenas];
            for(i=0;i<numDezenas;i++) tempJogo[i] = jogo7Dezenas[i];
            for(i=0;i<numDezenas;i++){
                contDupla = 0;
                for(j=i+1;j<numDezenas;j++){
            ...
            if(duques==1 && ternos==0){
                    preReqs++;
                    for(i=0;i<numDezenas;i++) jogo7Dezenas[i] = tempJogo[i];

            }
            // ---------------------------------------------------------------------------------
            // 7 ------  Permitir no maximo 2 numeros da mesma faixa de dezenas :ex, 22, 27  -------
            ...
            if(cont09<3 && cont1019<3 && cont2029<3 && cont3039<3 && cont4049<3 && cont5060<3) preReqs++;
            // ------------------------------------------------------------------------------------
            // ---------   O Jogo só passa se todos os 'PRE-REQUISITOS FOREM ATENDIDOS' -----------
            if(preReqs==7){
                ordenaJogo();
                quant++;
                if(jogo7Dezenas[numDezenas-1] <= 60){ // Gabiarra para imprimir corretamente,necessita de correção posterior
                    contGeral++;
